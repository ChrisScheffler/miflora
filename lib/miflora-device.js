...
class MiFloraDevice {
	...
	queryFirmwareInfo() {
		return timeout(10000, async (resolve, reject) => {
			...
			try {
				...
				logDebug('[%s] successfully queried firmware information: %o', this.address, firmwareInfo);
	...
	querySensorValues() {
		return timeout(10000, async (resolve, reject) => {
			...
			try {
				...
				logDebug('[%s] successfully queried sensor values: %o', this.address, realtimeData);
	...
	querySerial() {
		return timeout(10000, async (resolve, reject) => {
			...
			try {
				await this._setSeriaMode();
				const data = await this._readCharacteristic(UUID_SERVICE_DATA, UUID_CHARACTERISTIC_DATA);
				const dataValue = data.toString('hex');
				logDebug('[%s] successfully queried serial: %s', this.address, dataValue);
	...
	query() {
		return timeout(10000, async (resolve, reject) => {
			...
			try {
				const result = {};
				result.firmwareInfo = await this.queryFirmwareInfo();
				result.sensorValues = await this.querySensorValues();
